...
def generate_plots(N, mu, sigma2, S):
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title(f"Linear Fit: y = {slope: .2f} + {intercept:.2f}")
    plt.legend()
    plot1_path = "plot1.png"
    plt.savefig(f"assign6/laimao19-assignment-6/static/{plot1_path}")
    plt.close()
    ...
    plt.savefig(f"assign6/laimao19-assignment-6/static/{plot2_path}")
    plt.close()
